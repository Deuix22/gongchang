...
- `wechatTemplateId`: 可选，微信订阅消息模板ID，用于引导用户订阅（建议返回）
...
**⚠️ 重要提示**：
- `WECHAT_TEMPLATE_ID` 是**必须配置**的环境变量
- 如果未配置，后端会返回错误："未配置微信模板ID"
- 配置后需要重启后端服务才能生效

**配置步骤**：
1. 登录微信公众平台，在"功能" → "订阅消息"中获取模板ID
2. 将模板ID添加到 `.env` 文件中
3. 重启后端服务
4. 验证配置：发送测试通知，检查是否还有"未配置微信模板ID"错误

### 12.8 常见错误处理

| 错误信息/错误码 | 说明 | 处理方式 |
|----------------|------|----------|
| **未配置微信模板ID** | 环境变量 `WECHAT_TEMPLATE_ID` 未设置 | 1. 在 `.env` 文件中添加 `WECHAT_TEMPLATE_ID`<br>2. 重启后端服务 |
| 40037 | 模板ID无效 | 检查模板ID配置是否正确，确认模板ID是否在微信公众平台中有效 |
| 43101 | 用户拒绝接收消息 | 用户需要重新订阅，记录失败但不影响其他用户 |
| 47003 | 参数错误 | 检查模板数据格式是否正确，确认字段名与模板配置一致 |
| 40001 | access_token无效 | 重新获取access_token，检查 AppID 和 AppSecret 是否正确 |
...
#### 12.10.1 后端返回成功但用户未收到通知

**问题现象**：
- 后端返回 `{success: true, sent: 1, failed: []}`
- 但用户在小程序中未收到通知

**可能原因及解决方案**：

1. **用户未订阅订阅消息模板**
   - **原因**：微信订阅消息需要用户主动订阅才能接收
   - **解决方案**：
     - 在用户绑定微信时，引导用户订阅消息模板
     - 使用 `wx.requestSubscribeMessage` API 请求用户授权
     - 参考代码示例（见下方）

2. **后端未正确捕获微信API错误**
   - **原因**：后端可能没有正确处理微信API返回的错误
   - **解决方案**：
     - 检查后端日志，查看微信API的实际返回
     - 确保后端正确处理 `errcode !== 0` 的情况
     - 即使微信返回错误，也应该在 `failed` 数组中记录

3. **模板消息格式不正确**
   - **原因**：模板数据格式与模板配置不匹配
   - **解决方案**：
     - 检查模板字段名是否与配置一致
     - 检查字段类型是否正确（thing、number、time等）
     - 检查字段内容长度是否符合限制

4. **access_token 无效或过期**
   - **原因**：access_token 可能已过期或无效
   - **解决方案**：
...
```javascript
// 在 handleBindWechat 函数中，绑定成功后引导订阅
// #ifdef MP-WEIXIN
  ...
  try {
    ...
    const subscribeResult = await new Promise((resolve) => {
      uni.requestSubscribeMessage({
        tmplIds: [process.env.WECHAT_TEMPLATE_ID || 'YOUR_TEMPLATE_ID'], // 模板ID
        success: (res) => {
    ...
    if (subscribeResult && subscribeResult[process.env.WECHAT_TEMPLATE_ID] === 'accept') {
      uni.showToast({
        title: '已订阅消息通知',
        icon: 'success'
      })
...
**注意**：
- `wx.requestSubscribeMessage` 需要在小程序环境中使用
- 用户可能拒绝订阅，需要处理这种情况
- 建议在绑定微信时提示用户订阅，而不是强制订阅
...
async function sendTemplateMessage(openid, data, accessToken) {
  try {
    console.log('准备发送模板消息:', {
      openid: openid.substring(0, 10) + '...', // 只记录部分openid
    ...
    console.log('微信API返回:', {
      ...
      msgid: response.data.msgid // 如果成功，会有msgid
    })
...
| 43101 | 用户拒绝接收消息 | 引导用户重新订阅 |
| 47003 | 参数错误 | 检查模板数据格式 |
| 41030 | 页面路径不存在 | 检查page参数是否正确 |

### 12.11 注意事项

1. **模板ID配置**:
   - **必须**在 `.env` 文件中配置 `WECHAT_TEMPLATE_ID`
   - 如果未配置，后端应返回明确的错误信息："未配置微信模板ID"
   - 配置后必须重启后端服务
...
4. **错误处理**:
   - **必须正确处理微信API返回的错误**
   - 即使微信返回错误，也应该在 `failed` 数组中记录
   - 记录详细的错误信息，包括 errcode 和 errmsg
   - 返回详细的错误信息，帮助前端和用户了解问题

...
> **背景**：前端「物料库存管理 → 物料入库」页（`pages/warehouse/inventory-inbound`）已完成扫码解析与提交。  
> **后端状态**：**已落地**（`POST /api/warehouse/materials/inbound`，流水 + 库存事务、`rawCode` 唯一）。可直接联调。  
> **本文档为双方契约**；库存查询另议。  
> **前端解析**：`utils/warehouse/reelIdParser.js`（按 `@` 拆 7 段；提交前按表单字段重拼 `rawCode`）  
> **日期**：2026-09-16
...
1. **`rawCode` 全局唯一**（DB UNIQUE 索引，强制）  
   - 同一码再次提交 → HTTP **409**，业务码 **`REEL_ALREADY_INBOUND`**，库存不加  
   - 前端三层拦截：本地已入库记忆 → 提交前 `GET inbound-records?rawCode=` → 后端 409 Modal「重复入库」  
2. 写入 **入库流水**（每次扫码一条记录）  
...
| 重复入库码 | `REEL_ALREADY_INBOUND` → Modal「重复入库」 |
| 参数错误码 | `INVALID_PAYLOAD` → Modal「数据校验失败」 |
| rawCode | 提交前由 7 段字段重拼，与后端一致性校验对齐 |

**联调步骤**

1. 登录后进入：仓储首页 → 物料库存管理 → 物料入库  
2. 扫码或粘贴示例 Reel ID →「解析填入」→「确认入库」→ 应 `200` / Toast「入库成功」  
3. 同一码再提交 → 应 `409` / Modal「重复入库」  
4.（可选）改数量后再提交：前端会重拼 `rawCode`，应仍能通过一致性校验（首次成功；若已入库则仍 409）

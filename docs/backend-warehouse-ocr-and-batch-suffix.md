...
| 箱单号后缀 `01H`/`02B` 等 | **建议配合** | 同上；完整 `boxNo`（含后缀）写入出货记录；匹配仍用前 8 位 |
...
完整箱单 = HFSYHFYZBU + 8位batchKey + 可选后缀 + 6位流水
例：
  HFSYHFYZBU26040149000001       → batchKey=26040149，流水=000001，无后缀
  HFSYHFYZBU2607055201H000001    → batchKey=26070552，后缀=01H，流水=000001
  （注意：后缀在流水号之前，对应批号 GR-HFYZBU2607055201H）
```

### 后端检查项

1. **入库**：`productionBatchNo` / 出货 `boxNo` 按原文完整存储（含后缀），不要截断。  
2. **出货比对 `lookup` / 余量汇总**：按 `batchKey`（前缀后前 8 位数字）聚合，**不要**要求与完整批号/箱单字符串完全相等才算同一批。  
3. **箱单匹配**：解析出前 8 位 = `batchKey` 即可命中；流水号与后缀仅作记录，不参与匹配键。  

若后端本来就是按 `batchKey` 匹配，则后缀相关**可不改接口**，只需确认不会因「批号/箱单多了 01H」而匹配失败。

前端查询已传：`boxNo`（完整，可含后缀） / `batchKey` / `productionBatchNo`（完整）。

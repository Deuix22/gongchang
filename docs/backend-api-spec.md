...
**说明**: 统一的账号密码登录接口，支持所有角色（admin、manager、leader）。所有用户都使用此接口登录。
...
**说明**: ⚠️ **已废弃**。请使用统一登录接口 `POST /api/auth/login`。保留此接口仅用于向后兼容。
...
**说明**: ⚠️ **已废弃**。系统已改为统一的账号密码登录方式，不再支持微信快捷登录。
...
### 4.2 提交/更新出勤记录（按天追加，不覆盖历史）

**接口**: `POST /api/leaders/{leaderId}/attendance`

**权限**: `leader`, `admin`

**说明**: 一次性提交整组数据，后端需按“leaderId + memberId + recordDate”作为唯一键，**同一天同一成员可以更新为当日最新提交，但不同日期的记录必须追加，不得覆盖历史**。

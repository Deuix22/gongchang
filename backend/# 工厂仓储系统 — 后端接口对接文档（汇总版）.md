...
| 类型 | 格式 | 示例 |
|------|------|------|
| 生产批号 | `GR-HFYZBU` + **8 位数字** | `GR-HFYZBU26040149` |
| 箱单号 | `HFSYHFYZBU` + **8 位数字** + **6 位流水号** | `HFSYHFYZBU26040149000001` |

- **批次匹配码 `batchKey`**：上述 8 位数字（例：`26040149`）。
- 箱单号中间的 8 位与生产批号末尾 8 位一致，后端用 `batchKey` 关联送货单明细的 `productionBatchNo`（建议规范存储为 `GR-HFYZBU{batchKey}`）。

...
**说明**: 所有用户（admin、manager、leader）都使用此接口登录。这是推荐的登录方式。
...
### 1.2 平台管理员账号密码登录（向后兼容）

**接口**: `POST /api/auth/login-admin`

**说明**: 仅允许平台管理员通过账号密码登录。此接口保留用于向后兼容，建议使用统一登录接口 `POST /api/auth/login`。
...
**说明**: 管理员创建新用户账号。所有用户（包括 admin、manager、leader）都使用账号密码登录。
...
```json
{
  ...
  "lastLoginAt": "2024-01-05T10:00:00.000Z"
}
```
...
**字段说明**:
- `nickName` (可选): 用户昵称，长度不超过20个字符
- `wechatCode` (可选): 微信小程序登录凭证，有效期5分钟，用于换取openId（绑定微信时使用）
- `wechatUserInfo` (可选): 微信用户信息对象（绑定微信时使用）
  - `nickName`: 微信昵称（可选）
  - `avatarUrl`: 微信头像URL（可选）
- `wechatOpenId` (可选): 设置为 `null` 或空字符串时，用于解绑微信
...
```json
{
  "error": {
    "code": "INVALID_PARAMS",
    "message": "至少需要提供一个要更新的字段（nickName、wechatCode 或 wechatOpenId）",
    "details": {}
  }
}
```
...
**注意事项**:
- 用户只能更新自己的信息
- 可以更新的字段：`nickName`（昵称）、`wechatOpenId`（通过 `wechatCode` 绑定或设置为 `null` 解绑）
- 其他字段（如 `role`、`department`）需要通过其他接口修改
- 更新后应返回完整的用户信息，包括 `wechatOpenId` 字段
- 微信登录凭证 `code` 的有效期只有5分钟，前端获取后应立即发送到后端
- 如果 `wechatCode` 无效或已过期，会返回微信API的错误信息
- 绑定微信后，`wechatOpenId` 字段会出现在响应中，前端可以根据此字段判断是否已绑定
- 解绑微信时，将 `wechatOpenId` 设置为 `null` 或空字符串，解绑后该字段在响应中为 `null`
- 解绑微信会同时清除 `wechatUnionId`，但不会清除 `avatarUrl`（保留用户头像）
...
**查询参数（可选）**:
- `memberId`: 仅查看指定成员
- `startDate` / `endDate`: 过滤开始时间（ISO 字符串）
  ...
  "records": [
    {
      ...
      "createdAt": "2024-01-01T07:50:00.000Z",
...
**字段规则**:
1. `records` 必须为数组且至少包含 1 条数据
2. `memberId`、`startTime`、`endTime`、`duration`、`shiftType` 必填
3. `duration` 单位为分钟，必须 > 0；`shiftType` 仅支持 `day` / `night`
4. 每个 `memberId` 仅保留一条记录，后续提交会覆盖旧值
5. `name`、`department` 可选，未传时将自动从成员/组长信息补齐
6. `submittedAt` 可全局传一次，也可在单条记录中覆盖；默认为当前时间
7. 后端会写入 `leaderName`、`lastUpdatedBy`、`notes` 等字段，便于导出
...
### 4.4 数据模型（AttendanceRecord）

| 字段            | 说明                                   |
|-----------------|----------------------------------------|
| `recordId`      | 出勤记录 ID（`att_xxx`）               |
| `leaderId`      | 组长 ID                                |
| `leaderName`    | 组长昵称或用户名                       |
| `department`    | 线别/部门                              |
| `memberId`      | 组员 ID（唯一键）                      |
| `name`          | 组员姓名                               |
| `shiftType`     | `day` / `night`                        |
| `startTime`     | 开始时间（ISO 字符串，UTC）            |
| `endTime`       | 结束时间（ISO 字符串，UTC）            |
| `duration`      | 时长（分钟）                           |
| `submittedAt`   | 组长最后提交时间                       |
| `lastUpdatedBy` | 最后更新人 ID                          |
| `notes`         | 备注（可选）                           |
| `createdAt`     | 创建时间                               |
| `updatedAt`     | 更新时间                               |
...
**说明**: 生成 Excel/CSV，返回导出任务 ID；后台异步生成文件。
...
**字段说明**:
- `format`: 可选，`xlsx`（默认）或 `csv`
- `includeHistory`: 是否把历史变更附加到导出的工作表
...
> ⚠️ 如果导出后台任务暂未开发完成或仍在排队，可以直接调用 `GET /api/leaders/{leaderId}/attendance` 获取同样的数据字段，由前端即时生成 Excel。
...
**权限**: `admin`、`manager`（需要认证）

**说明**: 当检测到异常数据时，向所有已绑定微信的管理员和admin用户发送微信订阅消息通知
...
**字段说明**:
- `targets` (必填): 目标用户列表，包含 `userId` 和 `wechatOpenId`
- `data` (必填): 通知数据
  - `anomalyCount` (必填): 异常数据总数
  - `summary` (可选): 异常类型统计摘要
  - `uploadedAt` (可选): 上传时间，格式：`YYYY-MM-DD HH:mm`
  - `details` (可选): 异常详情列表（最多5条）
...
```json
{
  "error": {
    "code": "INVALID_PARAMS",
    "message": "通知数据不完整，缺少 anomalyCount",
    "details": {}
  }
}
```
...
**注意事项**:
- 用户需要主动订阅微信模板消息才能接收通知
- 如果用户未订阅，发送会失败（errcode: 43101），但不会影响其他用户
- 后端会自动获取并缓存微信 access_token（有效期2小时）
- 模板ID需要在环境变量 `WECHAT_TEMPLATE_ID` 中配置
- 建议在用户绑定微信时引导用户订阅消息
- 微信对模板消息发送有频率限制，建议实现发送队列避免短时间内大量发送
